...
def main():
    ...
    parser.add_argument('--tune_type', type=str, default='val')
    parser.add_argument('--device', type=str, default='cpu')

    args = parser.parse_args()
    data_path = os.path.join('data', args.ds_name)
    exp_path = os.path.join('expdir', args.ds_name)

    assert args.tune_type in ("cv", "val")
    assert args.model in ("mlp", "catboost")

    # 将 objective 定义在 main 内部，以便访问 args 和 data_path
    def objective(trial):
        if args.model == "mlp":
        ...
        if args.tune_type == "cv":
            ...
            for fold in range(5):
                metrics_report = train_func(
                    exp_dir=None,
                    data_path=os.path.join(data_path, f'kfolds/{fold}'),
                    eval_type="real",
                    T_dict=T_dict,
                    params=params,
                    change_val=False,
