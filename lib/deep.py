...
from typing import Any, Callable, Literal, cast, Iterator

#  pip install "rtdl==0.14.0.dev5"
import rtdl
import torch
    ...
    while chunk_size != 0:
        try:
            ...
            if batch_size <= chunk_size:
                ...
                loss = None
                for chunk in _iter_batches(batch, chunk_size):   # zero.iter_batches(batch, chunk_size)
...
def _iter_batches(batch: Any, chunk_size: int) -> Iterator[Any]:
    """Split a batch into chunks of size <= chunk_size.
    Assumes batch is a tuple/list of tensors or a dict with same-length tensors.
    """
    if isinstance(batch, (tuple, list)):
        n = len(batch[0])
        for i in range(0, n, chunk_size):
            yield tuple(x[i:i + chunk_size] for x in batch)
    elif isinstance(batch, dict):
        keys = list(batch.keys())
        n = len(batch[keys[0]])
        for i in range(0, n, chunk_size):
            yield {k: v[i:i + chunk_size] for k, v in batch.items()}
    else:
