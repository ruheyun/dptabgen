...
class Timer:
    ...
    def run(self) -> 'Timer':
        """Start the timer (non-context-manager style)."""
        if self._is_running:
    ...
    @classmethod
    def launch(cls) -> 'Timer':
        """Create and start a timer in one call."""
        timer = cls()
        timer.run()
        return timer
    ...
    def duration(self) -> float:
        """Return elapsed time. If running, return current duration."""
        if self._is_running:
            return time.perf_counter() - self._start_time
        return self._elapsed_time

    # Optional: support context manager (if used elsewhere)
    def __enter__(self):
        return self.run()
        ...
        report['environment'].update(
            {
                ...
                'gpus': get_gpus_info(),  # zero.hardware.get_gpus_info()
...
def get_catboost_config(real_data_path, eval_type='catboost', tune_type='val'):
    ds_name = Path(real_data_path).name
    C = load_json(f'tuned_models/{eval_type}/{ds_name}_{tune_type}.json')
    return C