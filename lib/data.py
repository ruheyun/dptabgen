...
def read_pure_data(path, split='train'):  # unreverse train val test
    X_num = None
    X_cat = None
    if split == 'unreverse':
        y = np.load(os.path.join(path, f'y_gen_{split}.npy'), allow_pickle=True)
        if os.path.exists(os.path.join(path, f'X_gen_{split}.npy')):
            X_num = np.load(os.path.join(path, f'X_gen_{split}.npy'), allow_pickle=True)
    else:
