...
from lib import get_catboost_config
...
def train_mlp(
    ...
    if change_val:
        X_num_real, X_cat_real, y_real, X_num_val, X_cat_val, y_val = lib.read_changed_val(trans_data_path, val_size=0.2)
    ...
    if eval_type == 'merged':
        ...
        if not change_val:
            X_num_real, X_cat_real, y_real = lib.read_pure_data(data_path)
        ...
        if not change_val:
            X_num, X_cat, y = lib.read_pure_data(trans_data_path)
    else:
        raise "Choose eval method"

    if not change_val:
        X_num_val, X_cat_val, y_val = lib.read_pure_data(trans_data_path, 'val')
    X_num_test, X_cat_test, y_test = lib.read_pure_data(trans_data_path, 'test')
    ...
    if params is None:
        # params = lib.load_json(f"tuned_models/mlp/{Path(data_path).name}_cv.json")
        params = get_catboost_config(data_path, eval_type='mlp')
    ...
    if D.is_regression:
        ...
        y = {k: D.y[k].astype(np.int64).squeeze() for k in D.y}
    ...
    if exp_dir is not None:
        lib.dump_json(report, os.path.join(exp_dir, "results_mlp.json"))
