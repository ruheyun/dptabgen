...
def train_tvae(
    ...
    cat_features = list(map(str, range(X_num_train.shape[1],
                                       X_num_train.shape[1] + X_cat_train.shape[1]))) if X_cat_train is not None else []
...
def sample_tvae(
    ...
    if X_num_train is not None:
        np.save(parent_dir / 'X_num_gen', X_num.astype(float))
    if X_cat_train is not None:
        np.save(parent_dir / 'X_cat_gen', X_cat.astype(str))
    y = y.astype(float)
    if lib.load_json(real_data_path / "info.json")["task_type"] != "regression":
        y = y.astype(int)
    np.save(parent_dir / 'y_gen', y)  # only clf !!!
