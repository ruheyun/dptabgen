...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def sample_time(self, b, device, method='uniform', phase=1):
        if method == 'importance':
            ...
            if phase == 1:
                t = torch.randint(self.dp_params['S'], self.num_timesteps, (b,), device=device).long()
                pt = torch.ones_like(t).float() / (self.num_timesteps - self.dp_params['S'])
            elif phase == 2:
                t = torch.randint(0, self.dp_params['S'], (b,), device=device).long()
                pt = torch.ones_like(t).float() / self.dp_params['S']
    ...
    def mixed_loss(self, x, out_dict, phase=1):
        b = x.shape[0]
        device = x.device
        t, pt = self.sample_time(b, device, 'uniform', phase=phase)
